...
| 元件 | 規格 |
|------|------|
| 主機板 | MSI B85M-G43（B85 晶片組；PCH DMI 上行實測 ~1300 MB/s；PCIe2.0 x1/x4 槽位於 PCH） |
| CPU | Intel Core i5-4570（4C/4T，Haswell） |
| 記憶體 | DDR3-1600 4 GB × 2 = 8 GB（雙通道） |
...
| 協定 | 程序 | 用途 | 適用時機 |
|------|------|------|----------|
| **P1 排水態（steady-state）** | 建卷 → 先寫 **≥64 G 排水**（丟棄）灌滿 SLC → `dmsetup remove`+重建歸零計數 → 寫 **16 G 量測** → `dmsetup status` 驗分布 → 讀 16 G | 可重現的 **TLC 穩態**數據 | 疊碟表、模型驗證（不受 SLC 干擾）；主表 S4 用 20 G 卷、疊碟用 100 G 卷（卷容量，量測寫入一律 16 G） |
...
> 8/14 晚第二批實測（`docs/data/slc_curve_{20g,100g}.txt`）在 **60s 回充協定**下
> （每 GB 之間空檔 60 s，屬 P3 類）顯示 S4 在 `[64:27:9:4]`（borrow off）下
> **全程平坦、無衰減可觀測**：20G carve 80 點平均 **3278**（範圍 2798–3413）、
> 100G carve 40 點平均 **3137**（2829–3357），均無趨勢。原因在於 6.3.1 的
> 「**A-bound 封頂**」——64:27:9:4 使瓶頸回歸 A 碟，A 以穩態 solo（~2064）為
> 天花板，聚合 1M 順序寫**從頭頂在天花板**（`solo_A×104/64 ≈ 3354`），SLC 增額
> 無從顯現。SLC 暫態只在「A 非瓶頸」的配置（如 8/12 冷態疊碟
> 1981→2661→2787→3091）才可觀測。**此圖反證了「排水協定消除 SLC 假象」的
> 設計意圖已達成**——即使 60s 回充讓 SLC 帶復原，A-bound 封頂仍使聚合寫入
> 穩定頂在天花板，量測不受 SLC 增額干擾。

![圖 F12 SLC 平坦曲線（figs/F12_decay.svg）](figs/F12_decay.svg)

> **圖 F12 說明**：藍/綠實線為 20G/100G carve 在 60s 回充協定下的逐 GB 實測點
> （平坦、無趨勢）；紅虛線為模型天花板 `solo_A×104/64 ≈ 3354`，實測與之吻合。
> 相比 8/12 冷態疊碟（SLC 假象，附錄 B.2 已退役）的跳動曲線，此圖證明
> **A-bound 配置下即使 60s 回充協定（最易顯現 SLC 增額）仍平坦**——量測穩定。
> 6.1.2 表格中 S4 主表（排水態 P1）與 F12（60s 回充 P3 類）皆頂在天花板，
> 交叉驗證兩協定下結果一致。

**為什麼不用 io_uring**：io_uring + dm + 深佇列會把吞吐「灌高」到不反映真實硬體上限（應用常見的誤區 [10]）；libaio 深度 32 在此平台已能飽和所有碟。論文所有數字以
...
> **圖 F9 說明**：主表 S1–S4/MIR 寫入吞吐階梯（P1/P3 協定，8/14 下午定案值）。
> S2 3547 > S3 3370 是 S3 撞上 DMI 牆（6.3.4），S4 3300 為 DMI-aware 權重下
> 瓶頸回歸 A 碟的排水態值（-1.6%，6.3.1）；MIR 457 為同步鏡像寫的雙寫成本。

> S4 的 `[64:27:9:4]` 為 DMI-aware 權重（3.3.3），排水態（P1）實測 **3300/4292**，
> 分布逐 chunk 吻合、零誤差。權重把 B+C+D 合計流量壓在 DMI 預算內：排水態下
> B+C+D 寫份額 ≈ **1269** MB/s（= 3300×40/104）< DMI 1300，瓶頸回歸 A 碟自身，
> `T = solo_A×104/64 ≈ 2064×104/64 ≈ 3354` 命中（偏差 -1.6%，6.3.1）——這正是
> DMI-aware 權重的設計目標：不讓非 A 碟流量超過 DMI 上限。**注意**：A 承擔 ≈2031
> （= 3300×64/104）僅達 solo_A 的 ~98%，B+C+D 距 DMI 亦僅差 ~2.4%，兩者皆近
> 飽和；實測 3300 同時略低於 A 模型 3354 與 DMI 模型 3364，故「瓶頸回歸 A 碟」是
> 權重設計的**意圖**，而非與「DMI 飽和」可精確區分的判讀。冷態（fresh）S4 寫入為
> 3091（6.4.1，SLC 暫態所致）。**S4 以 borrow off 量測**：borrow-on 會使 D 份額
> 重導至 A 的 borrow area、加重 A 瓶頸而降到 ~2517（6.5 B4，非正確生產選擇）。
...
- **B@x4 S4 [64:27:9:4]（排水態、borrow off）實測 3300**：此權重是 DMI-aware（3.3.3），
  B+C+D 合計流量被壓在 DMI 預算內（≈1269 < 1300），瓶頸回歸 **A 碟自身**：
  `T = solo_A×104/64 ≈ 2064×104/64 ≈ 3354`，實測 -1.6% 命中——
  證實「權重設計正確時，模型由最快的受限資源預言」；也直接驗證了 3.3.3 的權重
  推導目的。（註：B+C+D 距 DMI 上限僅差 ~2.4%、A 亦未滿 solo（~98%），兩限制
  皆近飽和；DMI-aware 的意圖是「不讓非 A 碟流量超過 DMI 上限」而非宣稱 DMI 有
  大餘裕，見 6.1.3 註。S4 必須以 borrow off 量測：borrow-on 會把 D 份額重導至 A、
  加重 A 瓶頸，使寫入降至 ~2517，見 6.5 B4——非生產選擇。）

此組數據證明 (3.2) 不僅是上界，**在權重匹配時可達、權重失配時準確預言瓶頸**。

![圖 F10 瓶頸模型 vs 實測（figs/F10_model.svg）](figs/F10_model.svg)

> **圖 F10 說明**：主表各權重組的模型 vs 實測成對柱狀。權重匹配場景 S1–S4 全數
> 命中 ±5% 內，S4 排水態 **-1.6% 偏差**。模型含 DMI-aware 預算（3.3.3）：S3 的
> 模型值為 `solo_A + DMI ≈ 3364`；S4 的模型值為 `solo_A×104/64 ≈ 3354`（A 為
> 瓶頸、B+C+D 壓於 DMI 預算內）。

### 6.3.2 自動權重（auto_weight）

以 `scripts/auto_weight.sh` 窮舉（基底 2..40、每權重 floor/ceil ±1、cap 128、
依 8 G 平均 solo）求權重，冷態（P2）實測。**此批數據為 8/14 早、B@x1 拓撲**：
solo（8 G 平均）A=2056.5、B=413.1、C=518.0、D=220.3（Σsolo=3208）——與 6.3.1
主表（8/14 下午、B@x4 定案拓撲）屬**不同拓撲**，故權重體系不同（10:2/20:4:5/
56:11:14:6 vs 37:27/64:30:10/64:27:9:4），兩表不可混讀。

**表 6.7 auto_weight 結果（8/14 早，B@x1）**

| 卷 | auto-weight | 實測寫入(MB/s) | 模型 | 偏差 | 模型/Σsolo |
|----|-------------|---------------|------|------|------------|
| S2 | 10:2 | 2410 | 2468 | -2.4% | 99.9% |
| S3 | 20:4:5 | 2896 | 2982 | -2.9% | 99.8% |
| S4 | 56:11:14:6 | **3006** | 3194 | -5.9% | 99.6% |

> 註：「模型/Σsolo」欄為**模型上界**（依 (3.2) 預言的 max-min 值）對各卷 Σsolo
> 之比值，非實測值；**實測利用率**為 S2/S3/S4 各 97.6%/96.9%/93.7%
> （2410/2469、2896/2988、3006/3208），S4 的 -5.9% 偏差即此間隙（見下）。

- 寫入階梯**嚴格單調 2078 < 2410 < 2896 < 3006**；
- S4 的 3006 對等權重 6:1:1:1 的 1762（8/14 早實測，見 6.3.3），**提升 70.6%**——權重匹配的價值；
- S2/S3 命中 ±3% 內；S4 偏差 -5.9%（略超 ±5% 門檻）——auto-weight 的
  模型以「A 與 D 併聯」估計 3194、達 Σsolo 的 99.6%，但**實測僅達 93.7%**；
  枚舉權重落在碟速比附近的離散解使殘差屬枚舉顆粒度而非模型失效，主表的
  DMI-aware 手動權重（6.3.1）反而更精準（-1.6%）。
- 「模型上界/Σsolo ≥ 99.6%」指 (3.2) 預言的上限貼近 Σsolo，**實測利用率
  （93.7%）才是真實可達值**；模型仍準確指出窮舉方向（令各碟 `soloᵢ×ΣW/wᵢ`
  盡量相等、最大化最弱碟利用率）；
...
> 註：10:2:2:1 一行的模型用 **A 已寫 5.7 G 後、SLC 部分滿之 solo ≈ 1782**
> （非 cold solo_A≈2064），`1782×15/10 ≈ 2673`——同場連測使 A 的 SLC 帶
> 部分填充；以 cold solo_A 推算 `2064×15/10 ≈ 3096` 會高估，故表取該場
> 實測 solo 為準（RESULTS 8/13 晚）。

相差 **36%**，而兩者**分布都精確**（6.2.1）。此對照明確證明：**「權重正確」
（分布對）≠「性能好」；權重必須匹配實際速比**。這是 auto_weight 存在的
必要性證據。

**衰退複測（8/14 早，sdb solo 再降至 219.6）**：同一 6:1:1:1 下 S4 再降至
**1762**（模型 `D×9 ≈ 1976`、-10.8%），階梯 **1983 < 2384 < 2531 但 S4 掉回
1762**；auto_weight（9:2:2:1）把 S4 拉回 **2749**（+56%）。同一等權配置的
實測隨 sdb 衰退從 1966（8/13）→ 1762（8/14）下滑，證實「權重靜態、硬體衰退」
時瓶頸從 A 轉移至 D——正是 auto_weight 的動機（權重靜態即失配）。
6.6 限制 5 亦指出此為規模/動態情境的未驗證邊界。
...
**數學證明**（3.3.3）：`S3_max = solo_A + DMI ≈ 2064 + 1300 = 3364`，
`S2_max = solo_A + solo_B ≈ 3590`。故 **B85 上「a+b+c 三碟寫入」不會超過
「a+b 兩碟」**——C 只加容量、不加寫入吞吐。這是硬體（DMI 上行頻寬）的天花板，
**不是 driver 缺陷**；權重 [64:30:10] 已把吞吐頂到天花板（實測 3370 ≈ 模型 3364，
+0.2%）。
（註：DMI ~1300 為「多碟**同時**走 PCH」時的共享飽和值，非單碟硬上限——B 碟
solo 1522 即高於 1300、S2 中 B 份額 ≈1497（= 3547×27/64）亦然，故 S2 3547
不違反 DMI 牆；牆的效應在於 B+C（+D）**合計**流量被壓在 ~1300 內，如 S3 的
B+C ≈1296 已觸頂。）

**S4 驗證 DMI-aware 的設計目的**：四碟 [64:27:9:4] 下 DMI 預算（~1300）分配給
B+C+D 合計份額（40/104），排水態實測 B+C+D 寫流量 ≈**1269**（=3300×40/104）
< 1300、瓶頸回歸 A 碟（6.3.1 的 -1.6% 命中）。兩組數據互補：三碟版證明「權重
不修正時 DMI 是瓶頸」，四碟版證明「權重修正後非 A 碟流量不超過 DMI 上限」——
同一硬體限制的兩面。（註：B+C+D 距 DMI 僅差 ~2.4%、A 亦未滿 solo，兩限制皆近
飽和，見 6.3.1 註。）
...
同一平台以 `dm-striped`（LVM striped [9]，等權 1:1）與 TieredVol 加權條帶對照
（B@x4 + D 回池拓撲，順序 1M、20 G，實測）：

**表 6.11 順序大塊對照（vs LVM）**
...
- **4 K 小寫（順序，8/13 舊拓撲 B@x1）**：LVM 較快 ~1.32x——4 K bio 小於 chunk，
  繞過 WC（4.2.3）後，TieredVol 的 4 K 成本主要在條帶化拆分與放置分布；此為
  **已知取捨**（大塊並行贏、小塊輸），並列兩者才公平。**隨機 4 K 已在 B@x4
  新拓撲複測（6.4.2），TieredVol 反而 +16%**——差異源自測試類型（順序 vs 隨機）
  與拓撲（B@x1 vs B@x4），兩表不可混讀。

> 對照已在 B@x4 + D 定案拓撲上複測（8/14 晚 A3），結論「大塊 TieredVol 勝」
> 在兩種插槽拓撲下皆成立；LVM **順序** 4 K 小寫對照仍為舊拓撲數據（B@x1，
> LVM 較快 ~1.32x），**隨機** 4 K 對照為新拓撲（B@x4，TieredVol +16%，
> 6.4.2），標註於 6.6。
>
> 註：6.5 的 WC 4 K 612 MiB/s 為雙碟 `tv_s2` 配置之絕對吞吐（A.1.4(a)），
> 與本行四碟 B@x1 的相對對照（511 vs 676）配置不同，兩者不可互比；
> WC 併塊的最佳絕對值見 6.5。
...
**表 6.12 隨機 I/O 與混寫對照（8 G，B@x4 拓撲）**
...
> **4 K 結論依測試類型而異**：此處為**隨機 4 K**（B@x4 新拓撲），TieredVol 對
> LVM 為 +16%（LVM 隨機小寫遭 RMW/跨 strip 成本）；6.4.1 的 **順序 4 K 小寫**
> （B@x1 舊拓撲）LVM 較快 ~1.32x——「小塊誰贏」視順序/隨機與拓撲而定，大塊
> 順序與混寫則兩種拓撲皆勝。整體與 6.4.1 的取捨一致：**4 K 隨機 I/O 非條帶
> 對單碟的強項**，但**大塊順序與混寫仍勝**——明確劃出本論文適用邊界
> （對照 §1.3「聚焦順序吞吐」）。

## 6.5 專項測試

> **本節定位**：驗證「例外與容錯機制」的**正確性**——借調：一致性不變式與
> 持久化；鏡像：讀重試與成本取捨；WC：讀序語義與小寫可用性。本節**非頭條性能
> 宣稱**；性能增益屬條件性（見各項）。

- **借調（weight borrowing）**：慢碟高負載時整 block offload 至快碟借用區，
  4 G 寫入 verify err=0；`remove`/reload 後映射恢復（`.borrow` 持久化）——
  驗證 4.1 的四條一致性不變式。**本項宣稱以機制正確性為主；性能增益為條件性
  輔助——與拓撲相關**（B4，8/13/8/14 兩場）：
  - 慢碟 D 真為瓶頸（6:1:1:1 等權）時：借調 **+3%**（8/13 測）；
  - 定案權重 [64:27:9:4]（D 份額僅 3.85%、≈127 MB/s ≪ solo 229）時：D 天然
    非瓶頸、**無功可借，on≈off（雜訊內）**——兩者互補，證明借調只在「慢碟
    真為瓶頸」的拓撲有正效益。
  - **B4 補充（8/14 晚第二批）**：上述「中性」僅在 D 飽和背景成立。**D 閒置時
    borrow ON 反而淨損 16%（4 G）至 29%（8 G）**：m_s4b（borrow ON，
    area 2048 MB@A、watermark 256 KB）1M 寫 4G=2769、8G=2344（8G 時 2G 表滿＋
    unborrow 回寫更慢），vs 同權重 borrow OFF ~3300。原因：D 份額寫入被重導到 A 的 borrow
...
- **鏡像（MIR）**：寫入 **457 MB/s**（8/14 晚 A2，主表），達 mirror 碟 C 自身
  solo（≈517）的 ~88%——瓶頸是 mirror 碟（SATA）而非 primary 合併速率
  （8/12 對照場 primary 聚合 2390 vs mirror 卷 463、減損 ~81%，同源）；
  4 K 寫因 COW 成本 -52%；讀取 **0% 額外成本**（讀不複寫）；
  讀取 16 G **3799 MB/s**、分布 A:B=4:3 精確、C=0（6.1.3）。**讀重試 fail
  injection（C5）**：A 碟 1M 錯誤窗 → 自動 fallback mirror → byte 精確讀回
  （6.2.2）。
- **WC**：4 K 寫入修復後達 **612 MiB/s**（最終 build tv_s2；修復前 4 K 僅
  14→500 MiB/s）；大寫入路徑不受影響（1 M=2661 MiB/s 之量測對照）——WC 的價值在
  「把 4 K 級小寫併成大塊」；read-after-write 語義由 D7 驗證（立即讀回正確、
  fio 6 G verify 0 mismatch）。
...
2. **單平台與 DMI 牆**：所有量測在單一 B85 平台；DMI 上行 ~1300 MB/s 使三碟以上
    ...
    **DMI-aware 權重（3.3.3）正是此限制下的解法**：把非 A 碟合計
   流量壓進 DMI 預算（S4 下 B+C+D ≈1269 < 1300），使 S4 瓶頸回歸 A 碟（6.3.1）。
...
4. **公平性**：vs LVM 大塊對照已在 B@x4 + D 定案拓撲複測（6.4.1，~1.9x 寫、
   2.0–2.4x 讀）；**順序** 4 K 小寫對照仍為舊拓撲（B@x1）數據（LVM 較快
   ~1.32x，8/13），**隨機** 4 K 對照為新拓撲（B@x4，8/14，TieredVol +16%，
   6.4.2）——4 K 勝敗依測試類型與拓撲而異，比值會隨插槽改變，方向結論以
   大塊為主。
