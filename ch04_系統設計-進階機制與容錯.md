...
≈ max(primary 路徑時間, mirror 碟時間)。實驗中 M 卷寫入達 mirror 碟速率的
~81%（mirror 碟慢於 primary 合併速率所致）；4 K 寫因 COW 成本降幅較大（-52%）；
讀取幾乎 0% 額外成本（讀不複寫）。
...
| D10 | libaio 深度 32 | io_uring 深佇列 | 反映真實硬體上限，避免把數字「灌高」（6.1.2） |
