...
以三種量測協定（排水態/冷態/冷態＋空檔）驗證：模型命中率 ±5% 內，auto_weight 使
4 碟寫入達各碟之和的 99.6%、較等權重提升 52%；並揭露三個對儲存系統設計有普遍意義
...
cold with idle), the model predicts within ±5%; auto_weight achieves 99.6% of the
sum-of-solos write throughput (52% over equal weights). We also reveal three hardware
