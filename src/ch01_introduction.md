...
2. **條帶化假設失準**：為了讓多碟協同工作，軟體常採用**條帶化（striping）**——把資料
    依固定 chunk 輪流分配到各碟。經典的條帶化（硬體 RAID 0、Linux `dm-striped` [2]、LVM striped [9]）
...
> **本項定位為輔助/條件性機制**：主要驗證的是「不破壞確定性佈局」的一致性不變式
> 與持久化恢復（正確性層）；性能效益僅在慢碟確為瓶頸時成立、與拓撲相關
> （第六章 6.5）。主線宣稱仍為 P1、P2 與 P4。
...
- **輔助機制（借調）的定位**：權重借調是「確定性佈局之上的例外路徑」，宣稱以
  機制**正確性**（一致性不變式、持久化與 reload 恢復）為主；性能效益為**條件性
  輔助**，僅在慢碟確為瓶頸時成立、與拓撲相關（第六章 6.5）。
...
**貢獻 2：瓶頸性能模型與自動權重工具。**
   ...
   模型上界達 Σsolo 的 99%+（實測達 93.7%、比等權重提升 70.6%）；並處理共享匯流排
   （DMI）下的權重修正（DMI-aware，第三章 3.3.3）。權重匹配的排水態主表場景實測
   命中率 ±5% 內（四碟排水態 S4 更達 -1.6% 偏差）；auto_weight 的 S4 偏差 -5.9%
   屬枚舉顆粒度之例外；另於 6:1:1:1 等權的衰退複測觀察到 -10.8% 偏差（硬體衰退，
   第六章 6.3）。
...
per-block 表記錄目的地、可持久化、reload 後完整恢復（4 G 驗證 0 錯誤）。
  ...
  本項貢獻以**機制正確性**為宣稱；性能增益為**條件性輔助**（僅慢碟真為瓶頸時
  成立、與拓撲相關，見 6.5）。
...
**貢獻 5：四層測試架構與真實異質平台效能剖析。**
  ...
  (2) PCH DMI 上行頻寬牆（~1300 MB/s）使三碟併寫封頂於「solo_A + DMI」，C 碟只加容量；
      而以 DMI-aware 權重（3.3.3）壓低 DMI 碟份額後，四碟併寫瓶頸交還 A 碟
       （S4 排水態 -1.6% 命中），顯示「共享匯流排預算」須納入權重設計；
...
| G3 性能 | §3.3 瓶頸模型 | §5.8 `auto_weight.sh` | L4 fio | §6.3 權重匹配場景模型命中 ±5% 內 |
...
每一項宣稱（含三個對儲存設計有普遍意義的硬體洞見）；第七章總結全文並收束
各限制討論。
